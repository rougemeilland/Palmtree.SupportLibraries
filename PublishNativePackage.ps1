...
foreach ($projectDirectory in Get-ChildItem $baseDirectory -Directory | Where-Object {$_.Name -match "^Palmtree\..*\.Native"}) {
    $projectReleasePath = Join-Path $projectDirectory.FullName "bin/Release"
    if (Test-Path $projectReleasePath -PathType Container) {
        $nugetPackageFiles = Get-ChildItem $projectReleasePath -File | Where-Object {$_.Name -match "\.nupkg$"}
