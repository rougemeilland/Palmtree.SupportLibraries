...
namespace Palmtree.IO
{
    public class InProcessPipe
    {
            ...
            : SequentialInputByteStream
        {
            ...
            public Reader(InProcessPipe parent)
            {
                _parent = parent;
                _isDisposed = false;
            }
            ...
            : SequentialOutputByteStream
        {
            ...
            public Writer(InProcessPipe parent)
            {
                _parent = parent;
                _isDisposed = false;
            }
        ...
        private readonly ConcurrentByteQueue _queue;

        private Boolean _isDisposed;
        private StreamState _readerState;
        private StreamState _writerState;

        public InProcessPipe()
        {
            _isDisposed = false;
            _queue = new ConcurrentByteQueue();
            _readerState = StreamState.Initial;
            _writerState = StreamState.Initial;
        }

        public ISequentialInputByteStream OpenInputStream()
        {
            try
            {
                lock (this)
        ...
        public ISequentialOutputByteStream OpenOutputStream()
        {
            try
            {
                lock (this)
