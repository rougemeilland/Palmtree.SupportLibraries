...
try {
    ...
    Copy-Item (Join-Path $baseDirectory "Palmtree.IO.Console.Native.nuspec") -Destination $packagePath
    Copy-Item (Join-Path $baseDirectory "Palmtree.IO.Console.Native.targets") -Destination $packagePath
    Set-Location $packagePath
    $process = (Start-Process -FilePath "nuget" -ArgumentList "pack Palmtree.IO.Console.Native.nuspec -Properties Configuration=Debug -OutputDirectory ../../bin/Debug" -NoNewWindow -PassThru)
    ...
    Copy-Item (Join-Path $baseDirectory "Palmtree.IO.Console.Native.targets") -Destination $packagePath
    Set-Location $packagePath
    $process = (Start-Process -FilePath "nuget" -ArgumentList "pack Palmtree.IO.Console.Native.nuspec -Properties Configuration=Release -OutputDirectory ../../bin/Release" -NoNewWindow -PassThru)
    $handle = $process.Handle # おまじない https://stackoverflow.com/questions/10262231/obtaining-exitcode-using-start-process-and-waitforexit-instead-of-wait
    $process.WaitForExit()
    if ($process.ExitCode -gt 1) {
        Write-Host ("nuget の実行に失敗しました。: 終了コード: " + $process.ExitCode)
